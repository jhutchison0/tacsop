...
- **D9. Fixed lesson schema.** One line, greppable, in session docs: `LESSON (<status>): <claim> | evidence: <path or command> | owner: <artifact path or PARKED(n)> | verdict: <ADD|UPDATE|SUPERSEDE|NOOP>`. Failures and no-lesson dispositions are first-class (ReasoningBank: failure entries carried the same weight as successes; success-only accretion loses the tail).
...
### Wave 4 — Verified propagation (sketch; gated on Wave 2 bedded in for ≥ 2 sessions)
Propagation tiers (D5), acknowledgment ledger, canary set selection, O2 backlog disposition, `entries[0]` integrity fix in `propagate_doctrine.py` (test-first; this one is code), and the next cycle's doctrine entry (numbering per the ledger at that time) carrying WHETSTONE's conventions fleet-wide as TEMPLATE-COPY.
...
- No new downstream obligations before the canary validates A3.
