...
- [ ] [P1] Complete the `utils` → `tacsop` rename (ADR-0002, 2026-07-17). Remaining after the in-repo edits: (1) GitHub rename + `git remote set-url` on this machine; (2) local directory rename `~/projects/github/utils` → `tacsop` + move Claude Code project-state dir `~/.claude/projects/-home-jhutchison-projects-github-utils` → `...-tacsop` (do both between sessions — the live session is anchored to the old path); (3) repeat set-url + directory rename on every other machine with a clone (work, home, laptop); (4) run `propagate_doctrine.py` from the renamed directory next session so the notification carries the new path; (5) update any external CI/mirror references to the old URL.
- [ ] [P3] Next doctrine propagation cycle — bundle two small refinements when enough downstream value accumulates to justify a cycle: (1) **deep-modules discipline** added to `CLAUDE.md` Simplicity First + `python-prototyper.md` Step 4d (commit `8ad387e`, 2026-05-20); (2) **proword convention** added to `.claude/commands/task.md` — CONOPs/OPORDs get a 2–3 syllable noun handle; campaign-scaling doctrine preserved for future use (2026-05-23). NOTE 2026-07-17: deliberately NOT bundled into the rename entry — propagation-protocol Rule 4 (breaking changes ship standalone). Next cycle after the rename notification is consumed.
