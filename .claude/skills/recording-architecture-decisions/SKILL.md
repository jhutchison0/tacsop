...
6. **Commit and propagate.** ADR additions are doctrine candidates, so consider whether the decision affects downstream repos. If so, draft a `docs/doctrine-updates.md` entry. If not, just commit.
