...
- [ ] [P1] Complete the `utils` → `tacsop` rename (ADR-0002, 2026-07-17). DONE 2026-07-17 evening: GitHub rename; WSL box fully migrated (directory, both remotes set-url'd, Claude project-state dir moved). Remaining: (1) delete the two temporary compatibility symlinks created for the in-flight session once it ends — `~/projects/github/utils` and `~/.claude/projects/-home-jhutchison-projects-github-utils`; (2) repeat directory rename + `git remote set-url` + project-state-dir move on every other machine with a clone (work, home, laptop); (3) run `propagate_doctrine.py` from the tacsop path next session — and on whichever machine(s) host the full downstream roster (this WSL box discovers only 5 repos); (4) update any external CI/mirror references to the old URL; (5) optional hygiene: `git remote remove main` (redundant duplicate of origin).
- [ ] [P2] Cycle 8 — "Planning Doctrine" entry: **fully drafted in `docs/doctrine-updates-staged.md`** (staged 2026-07-17; bundles the retrospective-hardened FORMAT files, task.md promote-wiring + proword convention as co-dependents per Rule 1, and deep-modules riding along per Rule 3 with its own adoption row). SEQUENCING: the script extracts entries[0] only, so Cycle 7 (NAME CHANGE) must propagate FIRST; then follow the promotion checklist at the top of the staged file (set date, move entry to top of doctrine-updates.md, delete from staging, propagate).
